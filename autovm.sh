...
virt-sparsify --check-tmpdir=ignore --compress --convert qcow2 --format qcow2 $vol/$uidtemp.$format $vol/$uidtemp-sparsified.$format
...
format=qcow2
    ...
    mkdir -p /var/lib/libvirt/qemu/channel/target/domain-$uidtemp
    chown -R qemu:qemu /var/lib/libvirt/qemu/channel/target/domain-$uidtemp
    virt-clone \
    --connect qemu:///system \
    --original $uidtemp \
    --name $domain \
